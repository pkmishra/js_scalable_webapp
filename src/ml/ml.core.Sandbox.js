 ...
 *
 * Cascading Style Sheets (sandbox.css, defined by <ml.core.plugins.css>):
 *   - <ml.core.plugins.css.sandbox.css.addClass(element,name)>
 *   - <ml.core.plugins.css.sandbox.css.removeClass(element,name)>
 *
 * Document Object Model (sandbox.dom, defined by <ml.core.plugins.dom>):
 *   - <ml.core.plugins.dom.sandbox.dom.q(localId): DOM Element>
 *   - <ml.core.plugins.dom.sandbox.dom.element(name[,attributes[,childNodes]]): DOM Element>
 *   - <ml.core.plugins.dom.sandbox.dom.addListener(element,type,callback): Listener>
 *   - <ml.core.plugins.dom.sandbox.dom.removeListener(listener)>
 *   - <ml.core.plugins.dom.sandbox.dom.removeAllListeners()>
 *
 * Events for loose coupling with other modules (sandbox.events, defined by <ml.core.plugins.events>):
 *   - <ml.core.plugins.events.sandbox.events.subscribe(filter,callback)>
 *   - <ml.core.plugins.events.sandbox.events.unsubscribe(filter)>
 *   - <ml.core.plugins.events.sandbox.events.publish(event)>
 *
 * Asynchronous communication with a remote server (sandbox.server, defined by <ml.core.plugins.server>):
 *   - <ml.core.plugins.server.sandbox.server.send(url,data,receive)>
 *
 * Uniform Resource Locator, local navigation (sandbox.url, defined by <ml.core.plugins.url>):
 *   - <ml.core.plugins.url.sandbox.url.getLocation(): object>
 *   - <ml.core.plugins.url.sandbox.url.setHash(hash)>
 *   - <ml.core.plugins.url.sandbox.url.onHashChange(callback)>
 *
 * General utilities (sandbox.utils):
 *   - <ml.core.plugins.utils.sandbox.utils.has(object,property[,...]): boolean>
 *   - <ml.core.plugins.utils.sandbox.utils.is([...,]value[,type]): boolean>
 *   - <ml.core.plugins.utils.sandbox.utils.getTimestamp(): number>
 *   - <ml.core.plugins.utils.sandbox.utils.setTimeout(callback,delay): number>
 *   - <ml.core.plugins.utils.sandbox.utils.clearTimeout(timeoutId)>
 *   - <ml.core.plugins.utils.sandbox.utils.trim(string): string>
 *   - <ml.core.plugins.utils.sandbox.utils.log(message)>
 *   - <ml.core.plugins.utils.sandbox.utils.confirm(message): boolean>
 *
 ...
 * alternative plugins. See <ml.core.plugins.builder> for details.
 *
 * Authors:
...
define([
	...
	// Assign to ml.core.Sandbox
	// for backward-compatibility in browser environment
	mlCore.Sandbox = function (id) {
		...
		function getBox(createIfMissing) {
			...
			if (!has(box) && createIfMissing) {
				...
				factory = config.getOption("mlFactory", defaultFactory);
