...
define([
	...
	// Assign to ml.core.Module
	// for backward-compatibility in browser environment
	mlCore.Module = function (id, creator) {
		// Function: new Module(id,creator): Module
		// Constructor of a new Core Module.
		//
		// Parameters:
		//   id - string, the module identifier, e.g. 'ml.ui.myModule'
		...
		// namespace 'ml.ui', e.g. ml.ui.myModule while creator functions for Data
		// modules, with no user interface,  may be registered in the namespace
		// 'ml.data', e.g. ml.data.myModule.
		//
		// The sandbox API can be customized by configuring a different builder
		// to load additional or alternative plugins. See <ml.core.plugins.builder>
		// for details.
		...
		try {
			sandbox = getOption("mlBuilder", defaultBuilder).buildSandbox(id);
		...
		function start() {
			...
			var customFactory = getOption("mlFactory"),
				box;
		...
		function end() {
			...
			try {
				...
				var box = q("#" + sandbox.getId()),
					factory = getOption("mlFactory", defaultFactory);
